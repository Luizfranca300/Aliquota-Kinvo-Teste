...
namespace Aliquota.Controllers
{
    ...
    public class AliquotaController : ControllerBase
    {
        ...
        */
        ...
        //Put /V1/clientes{clienteId} -> Atualiza os saldo da carteira do cliente
        [HttpPut("clientes/{clienteId}/carteira")]
        public async Task<IActionResult> AtualizarSaldoCarteiraAsync(Carteira carteira , int clienteId)
        {
            if (carteira.ClienteId != clienteId)
            {
                return BadRequest("clienteId está invalido");
            }

            var result = await _aliquotaServico.AtualizarSaldoCarteiraAsync(carteira, clienteId);
        ...
        [HttpPost("clientes/{clienteId}/carteira")]
